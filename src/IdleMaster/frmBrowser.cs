...
namespace IdleMaster
{
    public partial class FrmBrowser : Form
    {
        ...
        private void WbAuth_DocumentCompleted(object sender, WebBrowserDocumentCompletedEventArgs e)
        {
            ...
            var globalHeader = htmlDoc.GetElementById("global_header");
            ...
            else if (url.StartsWith("javascript:") == false && url.StartsWith("about:") == false)
            {
                try
                {
                    var parentalNotice = htmlDoc.GetElementById("parental_notice");
        ...
        public static CookieContainer GetUriCookieContainer(Uri uri)
        {
            ...
            if (!InternetGetCookieEx(uri.ToString(), null, cookieData, ref datasize, InternetCookieHttpOnly,
                IntPtr.Zero))
            {
                if (datasize < 0)
                {
